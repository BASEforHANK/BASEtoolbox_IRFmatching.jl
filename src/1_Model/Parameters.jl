...
@label @latex_label @prior @flattenable @with_kw struct ModelParameters{T}
	...
	κw::T = 1/11 		| "kappa_w" 	| L"\kappa_w" 	| Gamma(gamma_pars(0.1, 0.02^2)...)  		| true  # Wage  adjustment costs (in terms of Calvo probs.)
	...
	ρ_A::T = 0.9 		| "rho_A" 		| L"\rho_A" 	| Beta(beta_pars(0.5, 0.2^2)...)        	| false  # Pers. of bond-spread
	σ_A::T = 1.01 		| "sigma_A" 	| L"\sigma_A" 	| InverseGamma(ig_pars(0.001, 0.02^2)...) 	| false  # Std of bond-spread shock

	ρ_Z::T = 0.9 		| "rho_Z" 		| L"\rho_Z" 	| Beta(beta_pars(0.5, 0.2^2)...)        	| false  # Pers. of TFP
	σ_Z::T = 1.01 		| "sigma_Z" 	| L"\sigma_Z" 	| InverseGamma(ig_pars(0.001, 0.02^2)...) 	| false  # Std of TFP

	ρ_ZI::T = 0.9 		| "rho_Psi" 	| L"\rho_\Psi" 	| Beta(beta_pars(0.5, 0.2^2)...)        	| false  # Pers. of TFP
	σ_ZI::T = 1.01 		| "sigma_Psi" 	| L"\sigma_\Psi"| InverseGamma(ig_pars(0.001, 0.02^2)...) 	| false  # Std of TFP

	ρ_μ::T = 0.9 		| "rho_mu" 		| L"\rho_\mu" 	| Beta(beta_pars(0.5, 0.2^2)...)        	| false  # Pers. of price markup
	σ_μ::T = 1.01 		| "sigma_mu" 	| L"\sigma_\mu" | InverseGamma(ig_pars(0.001, 0.02^2)...) 	| false  # Std of cost push shock

	ρ_μw::T = 0.9 		| "rho_muw" 	| L"\rho_{\mu w}"  | Beta(beta_pars(0.5, 0.2^2)...)        	| false  # Pers. of wage markup
	σ_μw::T = 1.01		| "sigma_muw" 	| L"\sigma_{\mu w}"| InverseGamma(ig_pars(0.001, 0.02^2)...)| false  # Std of cost push shock
	...
	Σ_n::T = 00.0  		| "Sigma_n" 	| L"\Sigma_N" 	| Normal(-100.0, 100.0) 					| false  # reaction of risk to employment

	# monetary policy
	ρ_R::T = 0.82 		| "rho_R" 		| L"\rho_R" 	|   Beta(beta_pars(0.75, 0.1^2)...)          	| true  # Pers. in Taylor rule
	σ_Rshock::T = 0.01  	| "sigma_Rshock"| L"\sigma_R" 	| InverseGamma(ig_pars(0.001, 0.02^2)...) 	| false  # Std R
	...
	γ_BRB::T = 0.0 | "gamma_BRB" | L"\gamma_B^\tau" | Gamma(gamma_pars(0.05, 0.038^2)...)  			| true # reaction of tax level to debt

	# fiscal policy
	γ_B::T = 0.0 		| "gamma_B" 	| L"\gamma_B" 	| Gamma(gamma_pars(0.1, 0.075^2)...)      	| false  # reaction of deficit to debt
	γ_π::T = 0.0 		| "gamma_pi" 	| L"\gamma_{\pi}"| Normal(0.0, 1.0) 						| false  # reaction of deficit to inflation
	γ_Y::T = 0.0 		| "gamma_Y" 	| L"\gamma_Y" 	| Normal(0.0, 1.0) 							| false  # reaction of deficit to output
	
	γ_BG::T = 1.0 | "gamma_Btau" | L"\gamma_B^\tau" | Gamma(gamma_pars(0.5, 0.25^2)...)   | true # reaction of tax level to debt
	γ_YG::T = 0.0 | "gamma_Ytau" | L"\gamma_Y_\tau" | Normal(0.0,  1.0) | true # reaction of tax level to output
	ρ_G::T = 0.0 | "rho_G" | L"\rho_G" |  Normal(0.0, 0.5) | true # Pers. in structural deficit
	ρ_Gsum::T = 0.0 | "rho_Gsum" | L"\rho_Gsum" | Beta(beta_pars(0.5, 0.2^2)...) | true # Pers. in structural deficit
	ρ_GMA::T = 0.0 | "rho_GMA" | L"\rho_GMA" | Beta(beta_pars(0.5, 0.2^2)...) | false # Pers. in structural deficit

	ρ_Gshock::T = 1e-8 | "rho_Gshock" | L"\rho_G" | Beta(beta_pars(0.5, 0.1^2)...) | false # Pers. in structural deficit
	σ_Gshock::T = 0.01 | "sigma_Gshock" | L"\sigma_Gshock" | InverseGamma(ig_pars(1.00, 0.2^2)...) | true # Std G
	ρ_Gshock2::T = 0.0 | "rho_Gshock2" | L"\rho_Gshock2" | Beta(beta_pars(0.0, 0.1^2)...) | false # Pers. in structural deficit
	σ_Gshock2::T = 0.00 | "sigma_Gshock2" | L"\sigma_Gshock2" | InverseGamma(ig_pars(0.001, 0.02^2)...) | false # Std G	
	...
	γ_Yτ::T = 0.0 		| "gamma_Ytau" 	| L"\gamma_Y_\tau"| Normal(0.0, 1.0) 						| false  # reaction of tax level to output
	...
	SSEQRATIO::T = 1.0  | "rho_tau" | L"\rho_\tau" | Beta(beta_pars(0.5, 0.2^2)...)        | false #
	mmin::T = -13.5  | "mmin" |  L"mmin"  |  _  | false # mmin
end
...
@with_kw struct NumericalParameters
	...
	mmin::Float64      	= m_par.mmin      # gridmin bonds
