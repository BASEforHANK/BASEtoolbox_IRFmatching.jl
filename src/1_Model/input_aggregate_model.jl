...
#------------------------------------------------------------------------------
    ...
    F[indexes.A]            = log.(APrime./exp(XSS[indexes.ASS]))              - m_par.ρ_A * log.(A./exp(XSS[indexes.ASS]))               # (unobserved) Private bond return fed-funds spread (produces goods out of nothing if negative)
    ...
    F[indexes.Glag2] = log(Glag2Prime) - log(Glag)
    F[indexes.Gshock2] = log(Gshock2Prime) - log(Gshock)
    ...
    F[indexes.RB]           = log(RBPrime) - XSS[indexes.RBSS] -
                            ...
                            ((1 - m_par.ρ_R) * m_par.γ_BRB).* (log(BPrime) - XSS[indexes.BSS]) -
    ...
    F[indexes.τprog]        = log(τprog) - m_par.ρ_P * log(τproglag)  - 
                            ...
                            (1.0 - m_par.ρ_P) * m_par.γ_BP * log(BgovgrowthPrime) - 
                            log(Tprogshock)
    ...
    F[indexes.av_tax_rate]  = log(av_tax_rate) - m_par.ρ_τ * log(av_tax_ratelag)  - 
                                ...
                                (1.0 - m_par.ρ_τ) * m_par.γ_Bτ * log(BgovgrowthPrime) 

    F[indexes.G] = log(G) - (m_par.ρ_Gsum .- m_par.ρ_G) * log(Glag)  - m_par.ρ_G * log(Glag2)  - (1.0 - m_par.ρ_Gsum) *(XSS[indexes.GSS]) -
    (1.0 - m_par.ρ_Gsum) * m_par.γ_YG * log(YREACTION) +
    (1.0 - m_par.ρ_Gsum) * m_par.γ_BG *  (log(Bgov) - log(Btarget))  - log(Gshock) - m_par.ρ_GMA*log(Gshock2)
        
    F[indexes.tauLS] = log(tauLS) .* exp(XSS[indexes.YSS]) - m_par.γ_Bτ2 * log(BgovgrowthPrime)

    F[indexes.KG]   = log(KGPrime) - XSS[indexes.KGSS]

    F[indexes.Btarget] = log.(BtargetPrime) - (m_par.ρ_Btarget * log.(Btarget) + (1.0 - m_par.ρ_Btarget) *
    (XSS[indexes.BtargetSS]))

    F[indexes.Bgov] = log.(BPrime) .- log.(BgovPrime)

    F[indexes.π] = log(BPrime .- (1.0 .- m_par.δ_B) .* B ./ qBlag ./ π .* qB) .- 
                   log(B ./ qBlag ./ π .- T .+ G .+ log(tauLS) .* exp(XSS[indexes.YSS])  + 
                       q*(log(KGPrime) - log(KG)).*exp(XSS[indexes.KSS]) - (r - 1.0)*log(KG).*exp(XSS[indexes.KSS]))
    ...
    # Asset market premia
    F[indexes.LPXA]         = log.(LPXA)                - 4 * (log((qPrime + rPrime - 1.0)/q)  - log.((1.0./qB - m_par.δ_B + 1.0)/πPrime))
    F[indexes.LP]           = log.(LP)         - (m_par.λ * 4*(log((qPrime + rPrime - 1.0)/q) - log((1.0./qB - m_par.δ_B + 1.0))/πPrime) + (1.0 - m_par.λ) * log(LPPrime))  # ex-ante liquidity premium
    ...
    F[indexes.K]            = log.(K- log(KG).*exp(XSS[indexes.KSS]))     - XSS[indexes.KSS]                                                        # Capital market clearing
